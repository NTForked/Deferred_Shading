...

{
  ...
  out.rows[0] = vec4( 2/(r-l),       0,       0, -(l+r)/(r-l) );
  out.rows[1] = vec4(       0, 2/(t-b),       0, -(b+t)/(t-b) );
  out.rows[2] = vec4(       0,       0, 2/(n-f), -(n+f)/(f-n) );
  out.rows[3] = vec4(       0,       0,       0,            1 );
...
std::ostream& operator << ( std::ostream& stream, mat4 const& a )
...
std::istream& operator >> ( std::istream& stream, mat4 & a )
