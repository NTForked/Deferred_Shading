...
GBuffer::GBuffer( unsigned int width, unsigned int height, int nTextures, int *textureTypes )

{
  ...
  baseTextureUnit = GL_TEXTURE0 + 1; // reserve texture unit 0 for object textures (in wavefront.cpp)
  ...
  for (int i = 0 ; i < numTextures; i++) {
    ...
    glTexImage2D( GL_TEXTURE_2D, 0, textureTypes[i], windowWidth, windowHeight, 0, GL_RGB, GL_FLOAT, NULL );
...
void GBuffer::BindTexture( int textureNumber, int textureUnit )
...
void GBuffer::UnbindTexture( int textureNumber, int textureUnit )
...

{
  ...
  for (int r=0; r<dim; r++)
	...
	printString( buffer, c*width+10, (dim-r+1-1)*height-20, windowWidth, windowHeight );
